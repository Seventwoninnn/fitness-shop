...
<div class="container-fulid content">
    <div class="funbtn">
        ...
        <a class="btn btn-info" href="jsp/admin/cartManage/cartAdd.jsp">增加</a>
    </div>
    <table class="table table-striped table-hover">
        ...
        <c:choose>
            <c:when test="${!empty cartList}">
                <c:forEach items="${cartList}" var="i" varStatus="n">
                    <tr>
                        ...
                        <td class="noClick">
                            ...
                            <a class="btn btn-info btn-xs"
                               href="jsp/admin/CartManageServlet?action=edit&id=${i.id}">修改</a>
                            <a class="btn btn-danger btn-xs" href="jsp/admin/CartManageServlet?action=del&id=${i.id}"
...
<script type="text/javascript">
    ...
    $(".homePage").click(function () {
        window.location = "${bsePath}jsp/admin/CartManageServlet?action=list&page=1";
    })
    $(".prePage").click(function () {
        window.location = "${basePath}jsp/admin/CartManageServlet?action=list&page=${pageBean.prePage}";
    })
    $(".nextPage").click(function () {

        window.location = "${basePath}jsp/admin/CartManageServlet?action=list&page=${pageBean.nextPage}";
    })
    $(".lastPage").click(function () {

        window.location = "${basePath}jsp/admin/CartManageServlet?action=list&page=${pageBean.pageCount}";
    })
    //控制页面跳转范围
    $(".page-go").click(function () {
        ...
        if (page > pageCount || page < 1) {
            ...
            window.location = "${basePath}jsp/admin/CartManageServlet?action=list&page=" + page;
        }
    })
    ...
    $("#batDel").click(function () {
        ...
        if (confirm("你确定要删除" + choices.length + "条吗？")) {

            window.location = "${basePath}jsp/admin/CartManageServlet?action=batDel&ids=" + ids;
