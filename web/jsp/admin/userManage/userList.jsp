				...
				<c:when  test="${!empty userList}">
					<c:forEach items="${userList}" var="i" varStatus="n">
						<tr>
							...
							<td>
								<a class="btn btn-default btn-sm" href="jsp/admin/UserManageServlet?action=detail&id=${i.id}">详情</a>
								<a class="btn btn-info btn-sm" href="jsp/admin/UserManageServlet?action=edit&id=${i.id}">修改</a>
								<a class="btn btn-danger btn-sm" href="jsp/admin/UserManageServlet?action=del&id=${i.id}" onclick="javascript:return confirm('确定要删除吗？');">删除</a>
							</td>
						</tr>
					</c:forEach>
				</c:when>
