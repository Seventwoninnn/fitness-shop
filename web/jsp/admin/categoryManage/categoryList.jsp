...
<h2 class="text-center">分类列表</h2>
<div class="container-fulid content">
    <div class="funbtn">
        <a id="batDel" class="btn btn-danger" href="javascript:void(0)">批量删除</a>
        <a class="btn btn-info" href="jsp/admin/categoryManage/categoryAdd.jsp">增加一级分类</a>
        <a class="btn btn-info" href="jsp/admin/CategoryManageServlet?action=categorytoadd">增加二级分类</a>
    </div>
    <table class="table table-striped table-hover">
        ...
        <c:choose>
            <c:when test="${!empty categoryList}">
                <c:forEach items="${categoryList}" var="i" varStatus="n">
                    <tr>
                        <td class="noClick"><input type="checkbox" name="choice" value="${i.id}">
                        <td>${i.id}</td>
                        <td>${i.cateName}</td>
                        <td>${i.cateParentId}</td>


                        <td class="noClick">
                            <a class="btn btn-info btn-xs"
                               href="jsp/admin/CategoryManageServlet?action=edit&id=${i.id}">修改</a>
                            <a class="btn btn-danger btn-xs"
                               href="jsp/admin/CategoryManageServlet?action=del&id=${i.id}"
            ...
            <c:otherwise>
                <tr>
                    <td colspan="8"><h4 class="text-center">当前无更多分类信息</h4></td>
                </tr>
            </c:otherwise>
        </c:choose>
    </table>
...
<script type="text/javascript">
    ...
    $(".homePage").click(function () {
        window.location = "${bsePath}jsp/admin/CategoryManageServlet?action=list&page=1";
    })
    $(".prePage").click(function () {
        window.location = "${basePath}jsp/admin/CategoryManageServlet?action=list&page=${pageBean.prePage}";
    })
    $(".nextPage").click(function () {

        window.location = "${basePath}jsp/admin/CategoryManageServlet?action=list&page=${pageBean.nextPage}";
    })
    $(".lastPage").click(function () {

        window.location = "${basePath}jsp/admin/CategoryManageServlet?action=list&page=${pageBean.pageCount}";
    })
    //控制页面跳转范围
    $(".page-go").click(function () {
        ...
        if (page > pageCount || page < 1) {
            ...
            window.location = "${basePath}jsp/admin/CategoryManageServlet?action=list&page=" + page;
        }
    })
    ...
    $("#batDel").click(function () {
        ...
        if (confirm("你确定要删除" + choices.length + "条分类吗？")) {

            window.location = "${basePath}jsp/admin/CategoryManageServlet?action=batDel&ids=" + ids;
