...
public class UserServlet extends HttpServlet {
    ...
    private void regeiste(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        if("".equals(username)){
            ...
            response.getWriter().print("<script type='text/javascript'>alert('用户名不能为空');window.location.href='/jsp/regesite.jsp'</script>");
            return;
        }
        if("".equals(user_Password)){
            response.setContentType("text/html; charset=UTF-8");
            response.getWriter().print("<script type='text/javascript'>alert('密码不能为空');window.location.href='/jsp/regesite.jsp'</script>");
            return;
        }
        if(!(user_rePassword.equals(user_Password))){
            response.setContentType("text/html; charset=UTF-8");
            response.getWriter().print("<script type='text/javascript'>alert('两次输入密码不相同');window.location.href='/jsp/regesite.jsp'</script>");
            return;
        }
        ...
        if(count>0){
            response.setContentType("text/html; charset=UTF-8");
            response.getWriter().print("<script type='text/javascript'>alert('该用户名已被使用，不能重复注册');window.location.href='/jsp/regesite.jsp'</script>");
            return;
        }else{
            ud.userAdd(user);
            response.setContentType("text/html; charset=UTF-8");
            response.getWriter().print("<script type='text/javascript'>alert('注册成功');window.location.href='/jsp/login.jsp'</script>");
