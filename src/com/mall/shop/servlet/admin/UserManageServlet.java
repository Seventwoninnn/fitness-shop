...
public class UserManageServlet extends HttpServlet {
    ...
    private void userBatDel(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        if (ud.batDelUser(ids)) {
            //提示批量删除用户成功
            request.setAttribute("userMessage", "批量删除用户成功!");
            userList(request, response);
        } else {
            //提示批量删除用户不成功
            request.setAttribute("userMessage", "批量删除用户不成功!");
            userList(request, response);
        }
    }
    ...
    private void userUpdate(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        //获取前端页面input标签中的内容，构造新的User对象，更新数据库表中的用户信息
        User user = new User(
        ...
        if (ud.userUpdate(user)) {
            //提示用户更新成功
            request.setAttribute("userMessage", "更新用户成功！");
            //再次返回userList页面
            userList(request, response);
        } else {
            //提示用户更新失败
    ...
    private void userAdd(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        if (ud.userAdd(user)) {
            //提示添加用户成功
            request.setAttribute("userMessage", "添加用户成功");
            userList(request, response);
        } else {
            //提示添加用户失败
            request.setAttribute("userMessage", "添加用户失败，请重新添加");
    ...
    private void userDel(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        //删除就是根据前端页面传入的用户id,调用UserDaoImpl类中的输出用户的方法
        //获取用户id
        int id = Integer.parseInt(request.getParameter("id"));
        UserDaoImpl ud = new UserDaoImpl();
        if (ud.delUser(id)) {
            //提示删除成功
            request.setAttribute("userMessage", "删除用户成功!");
            userList(request, response);
        } else {
            //提示删除失败
            request.setAttribute("userMessage", "删除用户失败！");
            userList(request, response);
        }
    }
